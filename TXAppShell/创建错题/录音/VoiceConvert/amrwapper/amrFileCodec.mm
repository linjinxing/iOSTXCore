...
// 从WAVE文件读一个完整的PCM音频帧
// 返回值: 0-错误 >0: 完整帧大小
long ReadPCMFrame(short speech[], FILE* fpwave, int nChannels, int nBitsPerSample)
{
	long nRead = 0;
	int x = 0, y=0;
//	unsigned short ush1=0, ush2=0, ush=0;
	...
	{
		nRead = (long)fread(pcmFrame_8b1, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
		...
		{
			nRead = (long)fread(pcmFrame_8b2, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
...
//	}
    ...
    // 方式二：只要文件流中的新入数据够一个buffer的容量，就将其读出
	...
	while (1) {
        long size = ReadPCMFrame(speech, fpwave, nChannels, nBitsPerSample);
        if (isRecord) {
            if (size < 160) {
