...
- (void)viewDidLoad
{
    ...
    //取消拍摄的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dismissMAImagePickerController) name:@"cancleTakePhoto" object:nil];
    ...
    if (_sourceType == MAImagePickerControllerSourceTypeCamera && [UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        ...
        [_cameraToolbar setBarTintColor:[UIColor blackColor]];
...
//        cancelButton.frame = CGRectMake(10,0, 100, 40);
//        [cancelButton setTitle:@"取消拍照" forState:UIControlStateNormal];
//        cancelButton.titleLabel.font =[UIFont systemFontOfSize:14];
//        [cancelButton addTarget:self action:@selector(dismissMAImagePickerController) forControlEvents:UIControlEventTouchUpInside];

        UIImage *cameraButtonImage = [UIImage imageNamed:@"takepic"];
...
//        [_cameraToolbar addSubview:cancelButton];
