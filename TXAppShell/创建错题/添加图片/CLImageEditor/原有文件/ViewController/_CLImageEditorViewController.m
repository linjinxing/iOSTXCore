...
@property (nonatomic, copy) NSString* backupImagePath;
@end
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        for (CLImageToolInfo* tool in self.toolInfo.subtools) {
            if ([tool.title isEqualToString:@"智能处理"]) {
                tool.available = NO;
                break;
            }
        }
...
- (id)initWithImage:(UIImage*)image delegate:(id<CLImageEditorDelegate>)delegate
{
    ...
    if (self){
        ...
        self.backupImagePath = [_originalImage save2TempWithName:NSStringFromClass([self class])];
...
- (void)viewDidLoad
{
    ...
    if(_imageView==nil){
        ...
        LJXPerformBlockAsyn(^{
            _originalImage = [_originalImage thresholdImage];
...
- (void)tappedMenuView:(UITapGestureRecognizer*)sender
{
    ...
    void(^setToolState)(NSString* , NSString* ) = ^(NSString* enabledTitle, NSString* disableTitle){
        for (CLImageToolInfo* tool in self.toolInfo.subtools) {
            if ([tool.title isEqualToString:disableTitle]) {
                tool.available = NO;
            }else if ([tool.title isEqualToString:enabledTitle]) {
                tool.available = YES;
            }
        }
    };
    ...
    if ([view.title isEqualToString:@"原图"]) {
        setImage([UIImage imageWithContentsOfFile:self.backupImagePath]);
        setToolState(@"智能处理", @"原图");
        [self setMenuView];
    }
    else if ([view.title isEqualToString:@"智能处理"]) {
        setImage([_originalImage thresholdImage]);
        setToolState(@"原图", @"智能处理");
        [self setMenuView];
    }else{
        view.alpha = 0.2;
        [UIView animateWithDuration:kCLImageToolAnimationDuration
